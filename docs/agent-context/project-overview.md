...
- Full Per-Project Profiles are opt-in and workspace-scoped. When disabled, projects inherit repo profile settings when present, otherwise global settings. When enabled, only selected override groups replace lower-priority values.
- Team-Shared Repo Profile File supports schema version 1 safe profile defaults only. It does not include custom regex rules, suppression rules, terminal exit-code rules, rule presets, rule import/export schema, Alert History, custom audio file paths, merge-policy UI, network behavior, telemetry, or terminal reflection changes.
...
Full Per-Project Profiles in **Error Monitor → Project Profile** let a workspace opt into selected project-specific overrides without changing global settings. `ResolvedSettingsResolver` starts from global `AlertSettings.State`, applies the optional repo profile from `project.basePath/.error-sound-alert.json`, then applies enabled workspace project override groups, and returns an effective copy without mutating global settings, repo profile data, or project workspace state. **Copy current global settings** seeds a project profile from current globals; **Reset project overrides** clears the workspace profile back to inheritance.

Team-Shared Repo Profile File lets teams commit safe defaults into the repository root. The file is read-only from the plugin's perspective: it is not auto-created, edited, or written. Missing file, invalid JSON, invalid schema, unknown fields, and invalid values fall back safely with warnings where applicable. Error Monitor shows repo profile status with reload/open-file actions, and Diagnostics reports status, schema, name, warning count, and precedence.
