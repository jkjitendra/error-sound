...
- Full Per-Project Profiles are opt-in and workspace-scoped. When disabled, projects inherit global settings. When enabled, only selected override groups replace global values and unresolved fields continue inheriting globals.
- Phase 9 keeps custom regex rules, suppression rules, terminal exit-code rules, rule presets, rule import/export, Alert History, and terminal integration global/application-level. There are no per-project rules, per-project presets, per-project history, repo-shared profile files, merge-policy UI, or per-run-configuration overrides yet.
...
Full Per-Project Profiles in **Error Monitor → Project Profile** let a workspace opt into selected project-specific overrides without changing global settings. `ResolvedSettingsResolver` starts from global `AlertSettings.State`, applies only enabled project override groups, and returns an effective copy without mutating global settings or project workspace state. **Copy current global settings** seeds a project profile from current globals; **Reset project overrides** clears the workspace profile back to inheritance. Diagnostics show active project profile override categories when an active project is available.
