    ...
    fun buildSnapshot(): Snapshot {
        ...
        return Snapshot(
            rows = listOf(
                ...
                "Terminal command filter mode" to TerminalCommandFilterMode.fromStored(state.terminalCommandFilterMode).displayName,
                "Terminal command filters" to state.terminalCommandFilters.size.toString(),
                "Terminal command suppressions" to state.terminalCommandSuppressions.size.toString(),
                "Terminal exit-code rules" to state.exitCodeRules.size.toString(),
                "Run configuration overrides" to "${state.runConfigurationOverrides.size} Run/Debug-only override(s)",
                "Rule presets" to "${RulePresetService.bundles.size} bundled preset bundle(s) available",
                "Rule import/export schema" to "Exports v4; imports v1, v2, v3, and v4",
            ...
            notes = listOf(
                ...
                "Run configuration overrides apply to Run/Debug executions only.",
                "Terminal command filters apply to terminal command completions only.",
                "Terminal command suppressions apply to terminal command completions only.",
